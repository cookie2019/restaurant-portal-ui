          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div>
              ...
              <select
                ...
                className="w-full px-3.5 py-2.5 rounded-lg bg-[#1a1a1f] border border-[#2a2a32] text-[14px] text-[#f0f0f5] focus:outline-none focus:border-[#6c63ff] transition-colors disabled:opacity-50"
              >
                <option value="">
                  {loadingSlots ? "Loading…" : slots.length === 0 ? "No slots" : "Select"}
                </option>
                {slots.map((s) => (
                  <option key={s.time} value={s.time}>
                    {formatTime(s.time)} ({s.tablesAvailable} left)
                  </option>
                ))}
              </select>
            </div>
